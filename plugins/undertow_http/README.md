# Undertow Plugin Configuration

This plugin provides deep insights into the inner workings of Undertow web application.

It acts as a monitoring tool, providing detailed information about various aspects of servlets:
...
>
> Plugin to instrument WildFly(JBoss) >= 27 you can find in module [undertow23_http](../undertow23_http)

## Table of Contents

- [Introduction](#introduction)
- [Configuration](#configuration)
  - [Ruleset for Undertow plugin](#ruleset-for-undertow-plugin)
...
Config path: [config/minimal/50main/undertow_http.xml](./src/main/resources/config/minimal/50main/undertow_http.xml)

### Ruleset for Undertow plugin

In order to instrument every interaction for all incoming requests in profiler Undertow23 plugin is executed based on
below rules.

All incoming requests are handled by **UndertowRequestHandler** which will start instrumentation on method entry and
...
Undertow plugin provides capabilities to instrument endpoints (IP addresses and ports) on which the Undertow server
listens for incoming requests.

| Class                           | Method                 | Execute-After Exception ( if "true") |
|---------------------------------|------------------------|--------------------------------------|
| io.undertow.server.Connectors   | executeRootHandler     | `dumpQueueWaitTime$profiler(p2)`     |

Undertow plugin instruments filters which are Similar to standard Java Servlet API.It orchestrates requests intercepted
