...
function get_option_value() {
  if [[ -z "${!1}" ]] ; then
    configFile="${QS_DIAGNOSTIC_FOLDER}/properties/${1}"
...
function apply_esc_configuration_to_X_JAVA_ARGS() {
  local java_agent=""
  if check_option_not_disabled QS_DIAGNOSTIC_ESC_ENABLED ; then
    write_esc_log 'Attaching execution statistics collector'

    local local_dump_location="${QS_DIAGNOSTIC_FOLDER}/localdump"
    write_esc_log "creating directory for local dump: ${local_dump_location}"
    mkdir -p "${local_dump_location}"

    local esc_buffers
    esc_buffers="$(get_option_value QS_DIAGNOSTIC_ESC_BUFFERS 50)"
    # QS_DIAGNOSTIC_ESC_TAG_LIMIT should be in range 100..5100 (or -1)
    local esc_tag_limit
    esc_tag_limit="$(get_option_value QS_DIAGNOSTIC_ESC_TAG_LIMIT 5000)"
    java_agent="${java_agent}-javaagent:${QS_DIAGNOSTIC_FOLDER}/lib/agent.jar"
    java_agent="${java_agent} -Dorg.qubership.profiler.agent.Profiler.MAX_BUFFERS=${esc_buffers}"
    java_agent="${java_agent} -Dorg.qubership.profiler.agent.Profiler.INITIAL_BUFFERS=${esc_buffers}"
    java_agent="${java_agent} -Dorg.qubership.profiler.agent.Profiler.DICTIONARY_TAG_TRIM_SIZE=${esc_tag_limit}"
  ...
  local prf_options=""
  if diagnostic_enabled && check_option_not_disabled QS_DIAGNOSTIC_DUMPS_ENABLED ; then
  ...
  if version_ge "${javac_version}" "11.0.0"; then
    export X_JAVA_ARGS="${java_agent} $(gc_log_opts_for_jdk_11_and_high) ${prf_options} ${X_JAVA_ARGS}"
  else
    export X_JAVA_ARGS="${java_agent} $(gc_log_opts_for_jdk_7_and_less) ${prf_options} ${X_JAVA_ARGS}"
  fi
...
send_crash_dump() {
  ...
  "${QS_DIAGNOSTIC_FOLDER}"/diagtools scan "$(diagnostic_logs_folder)"/*.hprof* ./core* ./hs_err*
}
