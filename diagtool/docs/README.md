...
- `diagtools heap zip upload`
  ...
  - zip parameter is optional. Zips heap file with default compression level. Compression level can be configured with
    QS_HEAP_DUMP_COMPRESSION_LEVEL environment variable:
    - NoCompression      = 0
    - BestSpeed          = 1
    - BestCompression    = 9
...
- `diagtools dump`
  - "dump" subcommand is responsible for collecting java thread dump (`jstack -l "{{.Pid}}"`) and
    ...
    QS_DIAGNOSTIC_THREADDUMP_ENABLED and QS_DIAGNOSTIC_TOP_ENABLED is responsible for turning on/off collecting
...
- `diagtools schedule`
  - "schedule" subcommand is responsible for collecting dumps(like dump subcommand),
    scanning(like scan subcommand) and cleaning logs located in QS_DIAGNOSTIC_LOGS_FOLDER by schedule.
...
- `diagtools zkConfig "${QS_DIAGNOSTIC_FOLDER}/zkproperties" esc.config QS_DIAGNOSTIC_ESC_ENABLED ...`
...
- QS_DIAGNOSTIC_FOLDER path - to diagnostic folder. Default is `/tmp/diagnostic`.
- QS_DIAGNOSTIC_LOGS_FOLDER - path to logs. Default is `/tmp/diagnostic/log`.
- LOG_FILE_SIZE - size of log file in MB. Default is 1.
- LOG_FILE_BACKUPS - number of log file backups. Default is 5.
- KEEP_LOGS_INTERVAL - logs located in QS_DIAGNOSTIC_LOGS_FOLDER rotation interval in days. Default is 2.
...
- QS_DIAGNOSTIC_THREADDUMP_ENABLED - used to check if thead dumps enabled.
- QS_DIAGNOSTIC_TOP_ENABLED - used to check if top dumps enabled.
...
- QS_DIAGNOSTIC_AGENT_SERVICE - diagnostic agent service name. Default is `nc-diagnostic-agent`.
- PROFILER_FOLDER -path to profiler folder. Default is `/app/ncdiag`
...
- QS_HEAP_DUMP_COMPRESSION_LEVEL - defines heap dump compression level. Default is `-1`.
