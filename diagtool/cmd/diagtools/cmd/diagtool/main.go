...
import (
	...
	"qubership-agent/diagtools/handlers"
	"qubership-agent/diagtools/log"
	"qubership-agent/diagtools/utils"
	"flag"
	"fmt"
	"os"
	"os/signal"
...
scan [list of file patterns]
   ...
   example: diagtools scan "${QS_DIAGNOSTIC_LOG_FOLDER}"/*.hprof* ./core* ./hs_err*
schedule
   collects dumps(like dump subcommand), uploads ".hprof.zip" and other found files, scans (like scan subcommand) and
   clears logs located in QS_DIAGNOSTIC_LOGS_FOLDER by schedule.
...
zkConfig 'path to zookeeper property file' 'zookeeper properties which are to be changed'
   ...
   example: diagtools zkConfig "${QS_DIAGNOSTIC_FOLDER}/zkproperties" esc.config QS_DIAGNOSTIC_ESC_ENABLED
`
...
func scanModifier(baseCtx context.Context, done chan struct{}) {
	// E.g.: ./diagtools scan "${QS_DIAGNOSTIC_LOG_FOLDER}"/*.hprof* ./core* ./hs_err*
...
Usage:
   ...
   example: diagtools consulCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED
`
...
func consulConfigModifier(baseCtx context.Context, done chan struct{}) {
	// E.g.: ./diagtools consulCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED ...
...
Usage:
   ...
   The first parameter is a path to property file.
   The second and further ones are the properties which are to be changed.
   example: diagtools serverCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED
`
...
func cmdConfigServerModifier(baseCtx context.Context, done chan struct{}) {
	// E.g.: ./diagtools serverCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED ...
...
Usage:
   ...
   The first parameter is a path to property file.
   The second and further ones are the properties which are to be changed.
   example: diagtools zkCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED
`
...
func cmdZkConfigModifier(baseCtx context.Context, done chan struct{}) {
	// E.g.: ./diagtools zkCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED ...
