...
func TestIsLongListeningEnabled(t *testing.T) {
	...
	os.Setenv("QS_DIAGNOSTIC_JSTACK_LONG_LISTENING_ENABLED", "true")
	assert.True(t, IsLongListeningEnabled())
	os.Setenv("QS_DIAGNOSTIC_JSTACK_LONG_LISTENING_ENABLED", "false")
	assert.False(t, IsLongListeningEnabled())
	os.Setenv(NcDiagJStackLongEnabled, "true")
	assert.True(t, IsLongListeningEnabled())
	assert.Equal(t, "QS_DIAGNOSTIC_JSTACK_LONG_LISTENING_ENABLED", NcDiagJStackLongEnabled)
}

func TestIsThreadDumpEnabled(t *testing.T) {
	assert.True(t, IsThreadDumpEnabled(testCtx))
	os.Setenv("QS_DIAGNOSTIC_THREADDUMP_ENABLED", "true")
	assert.True(t, IsThreadDumpEnabled(testCtx))
	os.Setenv("QS_DIAGNOSTIC_THREADDUMP_ENABLED", "false")
	assert.False(t, IsThreadDumpEnabled(testCtx))
	os.Setenv(NcDiagThreadDumpEnabled, "true")
	saveFile("QS_DIAGNOSTIC_THREADDUMP_ENABLED", []byte("true"))
	assert.True(t, IsThreadDumpEnabled(testCtx))
	saveFile("QS_DIAGNOSTIC_THREADDUMP_ENABLED", []byte("false"))
	assert.False(t, IsThreadDumpEnabled(testCtx))
	deleteFile("QS_DIAGNOSTIC_THREADDUMP_ENABLED")
	assert.True(t, IsThreadDumpEnabled(testCtx))
	assert.Equal(t, "QS_DIAGNOSTIC_THREADDUMP_ENABLED", NcDiagThreadDumpEnabled)
}

func TestIsTopDumpEnabled(t *testing.T) {
	assert.True(t, IsTopDumpEnabled(testCtx))
	os.Setenv("QS_DIAGNOSTIC_TOP_ENABLED", "true")
	assert.True(t, IsTopDumpEnabled(testCtx))
	os.Setenv("QS_DIAGNOSTIC_TOP_ENABLED", "false")
	assert.False(t, IsTopDumpEnabled(testCtx))
	saveFile("QS_DIAGNOSTIC_TOP_ENABLED", []byte("true"))
	assert.True(t, IsTopDumpEnabled(testCtx))
	saveFile("QS_DIAGNOSTIC_TOP_ENABLED", []byte("false"))
	assert.False(t, IsTopDumpEnabled(testCtx))
	deleteFile("QS_DIAGNOSTIC_TOP_ENABLED")
	os.Setenv(NcDiagTopEnabled, "true")
	assert.True(t, IsTopDumpEnabled(testCtx))
	assert.Equal(t, "QS_DIAGNOSTIC_TOP_ENABLED", NcDiagTopEnabled)
}
...
func TestDiagFolder(t *testing.T) {
	...
	os.Setenv("QS_DIAGNOSTIC_FOLDER", "/app/ncdiag/test")
...
func TestDumpFolder(t *testing.T) {
	...
	os.Setenv("QS_DIAGNOSTIC_LOGS_FOLDER", "/tmp/diagnostic/test")
	assert.Equal(t, "/tmp/diagnostic/test", DumpFolder())
	os.Setenv("QS_DIAGNOSTIC_LOGS_FOLDER", "")
}
