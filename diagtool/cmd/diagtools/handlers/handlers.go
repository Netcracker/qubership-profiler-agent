...
import (
	"context"
	"errors"
	actions2 "qubership-agent/diagtools/actions"
	config2 "qubership-agent/diagtools/config"
...
func HandleScanCmd(ctx context.Context, args []string) (err error) {
	// scan "${QS_DIAGNOSTIC_LOG_FOLDER}"/*.hprof* ./core* ./hs_err*
	if len(args) == 0 {
...
func HandleZkConfigCmd(ctx context.Context, args []string) (err error) {
	...
	zkCfg := config2.ZkCfg{}
	// zkCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED ...
...
func HandleConsulConfigCmd(ctx context.Context, args []string) (err error) {
	...
	// consulCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED ...
...
func HandleConfigServerCmd(ctx context.Context, args []string) (err error) {
	...
	// serverCfg "${QS_DIAGNOSTIC_FOLDER}/properties" esc.config QS_DIAGNOSTIC_ESC_ENABLED ...
