...
Deletion also waits for the call-index partition drop (`03-lifecycle.md` §3.9, gate (b)), which itself follows the hot retention and eviction cadence, so the effective purge lag is `max(WAL_PURGE_GRACE, hot-index lag)` — not the grace alone. Under a sustained reconnect storm that distinction governs the PV: tracked pod-restarts accumulate at `restart rate × purge lag`, and the load campaign measured purge eligibility itself degrading as the backlog aged, leaving the set unbounded (`load-testing-report.md` §8, `runs/20260717T133845Z-t5-reconnect-storm`). That lag applies to pod-restarts with real data. A near-empty pod-restart — its on-PV directory at or under `PROFILER_WAL_PURGE_FAST_MAX_BYTES` — purges after the grace alone, skipping the partition-drop wait (`03-lifecycle.md` §3.9 step 18a), which bounds the eligible backlog at `restart rate × WAL_PURGE_GRACE` regardless of hot-window drift; restarts over the floor keep the full lag.
...
**Growth during live pod-restart:** the dictionary is append-only (§1 V3). If a collector crashes and recovers mid-flight, the WAL is replayed and the dense word array is rebuilt from the word id each record carries, so the read side sees an array of the same length and the ETag — the word count (`02-read-contract.md` §2.6) — comes back identical. Clients revalidating an old ETag get the fresh state.
...
**Pending-upload budget (backpressure):** `PROFILER_PENDING_UPLOAD_MAX_BYTES` (default 2 GB) bounds the backlog the PV holds while S3 falls behind: sealed parquet not yet confirmed in S3, the live call-index partitions, and the tracked pod-restarts' WAL files. Two gates hang off it. Once the pending parquet alone reaches half the budget, the seal loop pauses — the data stays in the WALs and segments, producing no new pending parquet, and the upload loop keeps draining. Once the whole backlog reaches the full budget, ingest refuses every `RCV_DATA` with `ACK_ERROR` before writing anything.

The two gates watch different quantities, so **which gate fires first depends on what dominates the backlog**, not on a fixed order. When pending parquet dominates (uploads stall while WAL churn is low), the seal gate trips first and ingest pauses only later. When WAL files and live partitions dominate (short buckets, reconnect churn, uploads draining what little parquet appears), the whole-backlog gate reaches the full budget before pending parquet alone can reach half of it — ingest pauses without `SealPaused` ever firing. The load campaign measured the second order on a 20-minute S3 outage: refusals began 10 minutes in with the seal gate silent throughout (`load-testing-report.md` §9, `runs/20260717T165508Z-t7-s3-outage`). Size the budget well clear of the steady WAL-plus-partitions level: at ~3× that level the same run left a replica hovering at the threshold after recovery, flapping brief refusals; the 2 GB default sat at ~11× on that stand. That refusal is data loss, and this contract states it honestly: the agent does NOT buffer and retry — `DefaultCollectorClient` treats `ACK_ERROR_MAGIC` as fatal, throws, drops the window it has not seen acknowledged, and reconnects as a fresh pod-restart (`06-wire-protocol-server.md` §6). The collector counts every refused byte on `profiler_ingest_refused_bytes_total`, and the shipped PrometheusRule alerts on any refusal — a bounded, visible loss instead of the PV running to `ENOSPC` and corrupting every pod-restart at once. TCP-level backpressure (pausing reads instead of refusing) was evaluated and rejected: the agent's 30-second ack timeout turns a paused read into the same reconnect-and-drop, with an application stall on top (remediation plan, decision on №1).
...
3. PUT the body to S3 with `Content-MD5`. On 200 OK, stamp `body_durable_at`: the body is now durable, and the row records that before the coupled manifest below so a manifest failure re-tests only the manifest, never re-PUTs the body (#825). Skip this PUT when `body_durable_at` is already set (a re-queued manifest-quarantined row).
4. Write or refresh that UTC day's pods manifest (§3.6). On a permanent rejection, quarantine the row (uploaded_at stays NULL) per the completion-couples-to-the-manifest rule above; on a transient failure, leave the row pending for the next pass.
5. Once the manifest is durable, set `uploaded_at` and decrement segment refcounts (§4.4) in one transaction. **The local file is not deleted here** — it serves the hot tier for `hot_retention` past upload (§6.3).
6. On any S3 error, retry with exponential backoff. The local file remains until upload succeeds.
...
**Sustained late data multiplies patch files.** A degraded agent path (latency, a stalled link) delays every call past its bucket's grace, so each seal pass re-emits patch files for closed buckets and the per-bucket object count climbs until compaction digests the burst: the load campaign measured 41–186 objects per `(bucket, retention_class)` against ~20 steady during agent-path fault windows (`load-testing-report.md` §9, `runs/20260717T235336Z-t7-agent-net`). This is the designed degradation, not a defect — the rows stay discoverable throughout, and `maintain` restores the object count once the backlog clears; expect compaction lateness for the duration.
...
A hard kill makes the lock visible during recovery: the dying process's flock can outlive it briefly, so the replacement pod's first start finds `collector.lock` held, exits, and kubelet restarts it; the second start recovers normally. The measured cost is one extra crash cycle per grace-0 kill — READY in 10–30 s including kubelet's backoff, flat across ten kill cycles (`load-testing-report.md` §8, `runs/20260717T142638Z-t5-restart`, `runs/20260717T152844Z-t5-crashloop`). This is the lock doing its two-writers job, not a failure mode; an in-process wait with a timeout would remove the extra restart (`load-testing-backlog.md`).
...
| `PROFILER_WAL_PURGE_FAST_MAX_BYTES` | `16MiB` | Near-empty floor of the fast-path WAL purge (§3.5; `03-lifecycle.md` §3.9 step 18a): a closed pod-restart whose directory is at or under it purges after the grace without waiting for the partition drop. `0` disables the fast path. |
