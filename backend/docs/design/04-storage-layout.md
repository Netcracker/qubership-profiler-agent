...
**The PVC must exceed the segment budget.** `PROFILER_CHUNKS_STAGING_MAX_BYTES` bounds only the trace/`sql`/`xml` segment files (`01-write-contract.md` §4.6). The WALs, the call-index SQLite files, and the pending sealed parquet are bounded separately by the pending-upload budget: `PROFILER_PENDING_UPLOAD_MAX_BYTES` (default 2 GB) gates ingest once pending parquet + call partitions + WAL files fill it (`01-write-contract.md` §4.6). Note the WAL coefficient: a live pod-restart's `calls.wal` is append-only for the life of its connection and purges only after upload + hot retention + the purge grace, so long-lived busy connections dominate this budget. The seal scratch, the hot-retention parquet (~1 GB), and the WALs of pod-restarts rebuilt by recovery (not tracked by the gate) sit outside both budgets. Size the PVC as segment budget + pending-upload budget + headroom for the rest (the `20Gi` default carries a `10GB` segment budget and a `2GB` pending budget). The RAM budget (`PROFILER_MEM_BUDGET`, `01-write-contract.md` §4.6) is a separate lever: `chunk_index` and seal-pass buffers are bounded by eviction, not by this PVC. Operators override per-environment via Helm values.
