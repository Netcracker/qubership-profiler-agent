...
public class Helper { // framework-agnostic helper. MUST be same in all subprojects (just COPY-PASTE it)
    ...
    void recursive(int level) {
        ...
        recursiveA(level);
        System.out.println("Got recursive of " + level + " level in " + (System.currentTimeMillis() - start) + " ms");
    }

    void recursiveA(int level) {
        tooLoong(500, random()); // 0.5s timeout
        if (level == 0) {
            tooLoong(7000, random()); // 7s timeout
            return;
    ...
    void recursiveB(int level) {
        tooLoong(500, random()); // 0.5s timeout
        if (level == 0) {
            tooLoong(10000, random()); // 10s timeout
            return;
        }
        recursiveA(level - 1);
    }
    ...
    void outOfMemory() {
        generateMegabytes(random());
    }

    void generateMegabytes(int megabytes) {
        long start = System.currentTimeMillis();
        System.out.println("Going to generate " + megabytes + " strings");
        long size = 0;
        ...
        System.out.println("Generated " + megabytes + " strings, " +
                "total size " + (size / 1024 / 1024) + " Mb in " + (System.currentTimeMillis() - start) + " ms");
    }
    ...
    String randomUrl() {
        switch (random(1, 4)) {
            case 1:
                return "http://localhost:8080/custom/err";
            case 2:
                return "http://localhost:8080/custom/health";
            case 3: {
                String status = Math.random() < 0.5 ? "UP" : "DOWN";
                return "http://localhost:8080/custom/health?status=" + status;
            }
            case 4: {
                int number = random(1, 10000);
    ...
    int random() {
        return random(5_000_000, 10_000_000);
    }
